...
1. **Detect** — `python scripts/kb.py conflict detect` (structured, exact subject+predicate), then `python scripts/kb.py conflict candidates` (stdlib lexical prefilter: surfaces paraphrased subjects/predicates the exact match is blind to; tags `DIFF_OBJECT_*`/`SAME_OBJECT` describe the string relation, not a verdict; the `~` scores are surface overlap, **never** a confidence value; candidates only, never link on the score alone; `--embed` adds optional model2vec), then a semantic pass by hand — **assume zero tool coverage**, the prefilter is a recall aid not a floor — for disjoint-vocabulary synonymy and entailment it can't reach. For a `SAME_OBJECT` pair, act on the printed `sources:` relation: **independent** (disjoint) is corroboration — **consolidate first** (ingest the other side's evidence onto the survivor's triple; `supersede` does not migrate evidence, so superseding first destroys the agreement), *then* the redundant duplicate may be superseded; **shared** (same source) is a restatement — supersede the redundant claim; **unknown** (a side has no evidence) — establish provenance before any supersede.
